...
for spect = 1 : nbr_spect
    ...
    L1 = wmaxlev(size(hyper_image1(:,:,spect)),'haar');
    L2 = wmaxlev(size(hyper_image2(:,:,spect)),'haar');
    decomp_level = min(L1,L2);
    fused_max_max = wfusimg(hyper_image1(:,:,spect),hyper_image2(:,:,spect),'db2',decomp_level,'max','max');
